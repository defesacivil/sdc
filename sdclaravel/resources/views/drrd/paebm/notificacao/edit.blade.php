...
@section('content')
    ...
    <div class='col-md-12'>
        ...
        {{ Form::open(['url' => 'pae/notificacao/update/'.$notificacao->id, 'method' => 'POST']) }}
        {{ Form::token() }}

        <div class='row p-2'>
            <div class='col'>
                {{ Form::label('Número SEI', '') }}:
                {{ Form::text('num_sei', $notificacao->num_sei, ['class' => 'form form-control', 'value' => old('num_sei')]) }}
            ...
            <div class='col'>
                ...
                {{ Form::date('dt_notificacao', \Carbon\Carbon::parse($notificacao->dt_notificacao)->format('Y-m-d'), ['class' => 'form form-control', 'value' => old('dt_notificacao')]) }}
            </div>
        </div>
        <div class='row p-2'>
            <div class='col'>
                ...
                {{ Form::select(
                    ...
                    $notificacao->prorrogacao,
                    ['class' => 'form form-control'],
                ) }}
            </div>
            <div class='col'>
                {{ Form::label('Data Devolutiva', '') }}:
                {{ Form::date('dt_devolutiva', \Carbon\Carbon::parse($notificacao->dt_devolutiva)->format('Y-m-d'), ['class' => 'form form-control', 'value' => old('dt_devolutiva')]) }}
            </div>
        </div>
        <div class='row p-2'>
            <div class='col'>
                {{ Form::label('obs', '') }}:
                {{ Form::textarea('obs', $notificacao->obs, ['class' => 'form form-control', 'value' => old('obs'), 'rows' => '3']) }}
            </div>
        </div>
